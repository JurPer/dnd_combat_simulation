...
class ResultContainer:
    ...
    def to_json(self):
        ...
        avg_num_rounds = statistics.mean(self.number_of_rounds) if self.number_of_rounds else 0
        avg_t1_deaths = (
            statistics.mean(self.number_of_player_deaths) if self.number_of_player_deaths else 0
        )
        at_least_1_t1_death = len(
            [value for value in self.number_of_player_deaths if value > 0]
        )
        perc_times_t1_won = 1 - statistics.mean(self.winning_teams) if self.winning_teams else 0
